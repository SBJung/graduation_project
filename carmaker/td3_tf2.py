...
class Agent:
    def __init__(self, alpha, beta, input_dims, tau, model_chkpt_dir, gamma = 0.99,
                 ...
                 n_actions = 2, max_size=3000000, batch_size=128, noise = 3.0):
        self.gamma = gamma
        self.tau = tau
        self.max_action = 5
        self.min_action = -5
    ...
    def choose_action(self, observation, episode):
        if episode < self.warmup:  # If episode is smaller than self.warmup
            gas = np.random.normal(scale=self.noise, size=(self.n_actions,))[0]
            steer = np.random.normal(scale=self.noise, size=(self.n_actions,))[1]   # Select Random Action
            #print("random, mu_gas1:", gas,"mu_steer1",steer)
            #print(observation)
        else:
            state = tf.convert_to_tensor([observation], dtype=tf.float32) # Else feed state to actor to get action
            print("mu :",self.actor(state))
            gas = self.actor(state)[0,0] 
            steer = self.actor(state)[0,1]  

            #print("tf, mu_gas2:", gas,"mu_steer2",steer)

        gas_prime = gas  # Add Noise to the action for exploration (원래 scale=0.01)
        gas_prime = tf.clip_by_value(gas_prime, self.min_action, self.max_action)
        
        steer_prime = steer  # Add Noise to the action for exploration (원래 scale=0.01)
        steer_prime = tf.clip_by_value(steer_prime, self.steer_min, self.steer_max)
        print("prime:",np.array([gas_prime,steer_prime]))
        return np.array([gas_prime,steer_prime])

    def remember(self, state, action, reward, new_state, done):
        self.memory.store_transition(state, action, reward, new_state, done)
        print("number of stacked data in Replay Buffer : {}".format(self.memory.mem_cntr))  # replay buffer에 얼마나 저장됐는지 보려고 넣음
